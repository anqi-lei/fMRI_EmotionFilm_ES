...
# Added for continuous valence rating
def run_continuous_slider(window, timer, duration, slider_params):
    """
    Show a continuous slider under the video during playback.
    Returns a list of {'time': t, 'rating': r} samples.
    """
    # create slider
    ...
    while True:
        ...
        # flip window
        window.flip()
        # record rating sample
        rating = slider.getRating() or slider.markerPos
        samples.append({'time': current_time, 'rating': rating})
        # allow escape
        ...
        if (trials[trialcount]['Stim_Cond'] == 'INSTR'): # anqi modified, Apr.15
            ...
            for stimcount in range(trial_parameter['num_stim']):
                ...
                if trial_parameter['Stim_Video_Type'] in trial_stim[stimcount]:
                    cur_stim = Display_Video(..., show_now=1, return_stimulus=True)
                    duration = cur_stim.duration  # in seconds
                    cur_stim.play()
                    rating_samples = run_continuous_slider(Experiment.window, timer, duration, {
                        'ticks': [1, 5, 9],
                        'labels': ['Negative', 'Neutral', 'Positive'],
                        'granularity': 0.1,
                        'pos': (0, -settings['window_size'][1]/4),
                        'size': (settings['window_size'][0]*0.7, 50),
                    })
                    cur_stim.stop()
                    trial_response['rating_samples'] = rating_samples
                    
                    # write rating_samples to a separate CSV
                    write_csv_rows(experiment_info['DataFile'].replace('.csv', '_valence.csv'),
                                    ['trial', 'time', 'rating'],
                                    [{'trial': trials[trialcount]['Trial_No'],
                                    'time': s['time'], 'rating': s['rating']} for s in rating_samples])
